...
try:
    subprocess.run(full_command, check=True, cwd=PROJECT_DIR, shell=True, capture_output=True, text=True)
    ...
    if os.path.exists(zephyr_exe_path):
        ...
        shutil.copy(zephyr_exe_path, OUTPUT_DIR + '/firmware.bin')
        print(f"To make PIO happy, also copied zephyr.exe as firmware.bin to {OUTPUT_DIR}")
    else:
